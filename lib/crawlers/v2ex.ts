...
interface V2EXJob {
  ...
  author: string;
  publishedAt: Date;
  isFromRemoteNode?: boolean; // Track if from remote node
}
...
export async function crawlV2EX(): Promise<{
  ...
  try {
    ...
    // Fetch from both nodes: jobs and remote
    const nodesToFetch = [
      { name: "jobs", pages: 3 },
      { name: "remote", pages: 3 }, // Remote work specific node
    ];
    const topicsWithSource: Array<V2EXApiTopic & { sourceNode: string }> = [];

    for (const node of nodesToFetch) {
      console.log(`\nFetching from node: ${node.name}`);

      for (let page = 1; page <= node.pages; page++) {
        try {
          console.log(`  Page ${page}...`);

          const response = await axios.get<{ result: V2EXApiTopic[] }>(
            `${V2EX_API_BASE}/nodes/${node.name}/topics?p=${page}`,
            {
              headers: {
          ...
          // Tag each topic with its source node
          topicsWithSource.push(
            ...topics.map((topic) => ({
              ...topic,
    ...
    console.log(`\nTotal fetched: ${topicsWithSource.length} topics from V2EX`);

    // Filter out job-seeking posts and collect valid jobs
    for (const topic of topicsWithSource) {
      ...
      // Include all non-seeking job posts
      jobsList.push({
        title: topic.title,
        ...
        isFromRemoteNode: topic.sourceNode === "remote",
      });
    }
    ...
    for (const jobData of jobsList) {
      try {
        ...
        // Determine remote type based on source node and content
        let remoteType: "FULLY_REMOTE" | "HYBRID" | "OCCASIONAL" = "OCCASIONAL";

        if (jobData.isFromRemoteNode) {
          // Jobs from remote node are always fully remote
          remoteType = "FULLY_REMOTE";
        } else {
          // For jobs node, check content for remote keywords
          ...
          remoteType = hasRemote ? "FULLY_REMOTE" : "OCCASIONAL";
        }
